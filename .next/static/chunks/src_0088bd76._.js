...
_s(MemoryMonitor, "X3XWpOLHAqxArW6nwoFdiyIySKM=");
_c = MemoryMonitor;
                        ...
                        const getPerformanceInfo = {
                            "NativeModuleStatus.useEffect.fetchStatus.getPerformanceInfo": async ()=>{
                                try {
                                    ...
                                    const actualPid = processInfo.pid || __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$build$2f$polyfills$2f$process$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].pid || Date.now() % 100000; // 임시 PID
                                    return {
                                        processUptime: Math.floor(Date.now() / 1000) % 3600,
                                        memoryUsage: {
                                            rss: actualMemoryUsage.rss || Math.floor(Math.random() * 200 + 50) * 1024 * 1024,
                                            heapTotal: actualMemoryUsage.heapTotal || Math.floor(Math.random() * 100 + 30) * 1024 * 1024,
                                            heapUsed: actualMemoryUsage.heapUsed || Math.floor(Math.random() * 80 + 20) * 1024 * 1024,
                                            external: actualMemoryUsage.external || Math.floor(Math.random() * 10 + 5) * 1024 * 1024,
...
function Settings({ onSave, initialSettings }) {
    _s();
    const { settings, isLoading, error, saveSettings, resetSettings } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$hooks$2f$useSettings$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useSettings"])();
    const { theme, isDarkMode, toggleDarkMode: themeToggleDarkMode, toggleTheme, setTheme } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$app$2f$components$2f$ui$2f$ThemeProvider$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useTheme"])();
    const [needsRestart, setNeedsRestart] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])(false);
    ...
    const handleSave = async ()=>{
        ...
        try {
            ...
            // 백엔드 연결 상태 확인
            if ("object" !== 'undefined' && window.electronAPI?.ipcRenderer) {
                console.log('🔌 Settings: Electron IPC를 통한 저장 시도');
                try {
                    // 백엔드에 설정 저장 요청
                    const result = await window.electronAPI.settings.updateMultiple(settings);
                    ...
                    if (result === true || result && result.success !== false) {
                        ...
                        if (!isGPUOnlyChange) {
                            ...
                            setTimeout(()=>{
                                if ("object" !== 'undefined' && window.electronAPI?.app?.restart) {
...
_s(Settings, "+F7vswMIyTwtUME6SK0pt1TJ7rA=", false, function() {
    return [
...
_s(HomePage, "iV85i6kcp1ViuBFsJLyGz8SBtvw=");
_c = HomePage;
var _c;
