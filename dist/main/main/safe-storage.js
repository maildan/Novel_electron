...
exports.storeSecureData = storeSecureData;
exports.loadSecureData = loadSecureData;
exports.deleteSecureData = deleteSecureData;
exports.listSecureDataKeys = listSecureDataKeys;
...
async function loadOrCreateEncryptionKey() {
    ...
    try {
        if (fs.existsSync(keyFilePath)) {
            ...
            encryptionKey = electron_1.safeStorage.decryptString(encryptedKey);
            ...
            // 암호화하여 저장
            const encryptedKey = electron_1.safeStorage.encryptString(encryptionKey);
 ...
 * 저장소 메타데이터 저장
 */
async function saveStorageMetadata() {
    try {
        const metadataPath = path.join(STORAGE_DIR, 'metadata.json');
...
function encryptData(data) {
    ...
    try {
        ...
        // 암호화
        const cipher = crypto.createCipheriv(ENCRYPTION_CONFIG.algorithm, Buffer.from(encryptionKey, 'hex'), iv);
 ...
 * 데이터 복호화
 */
function decryptData(encryptedData, parseJson = true) {
    if (!encryptionKey) {
        throw new Error('암호화 키가 초기화되지 않았습니다.');
    }
    try {
        ...
        // 암호화된 데이터 추출
        const encryptedContent = encryptedData.slice(ENCRYPTION_CONFIG.ivLength, encryptedData.length - 16);
        // 복호화
        const decipher = crypto.createDecipheriv(ENCRYPTION_CONFIG.algorithm, Buffer.from(encryptionKey, 'hex'), iv);
 ...
 * 백업 생성
 */
async function createBackup(safeKey) {
    try {
