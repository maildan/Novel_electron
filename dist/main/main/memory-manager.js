...
class AdvancedMemoryManager {
    ...
    async initialize() {
        try {
            ...
            (0, utils_1.debugLog)('고급 메모리 관리자 초기화 완료');
        }
        catch (error) {
            (0, utils_1.errorLog)('메모리 관리자 초기화 중 오류:', error);
            this.isInitialized = false;
        }
    }
    /**
    ...
    async loadSettings() {
        try {
            if (fs.existsSync(this.configPath)) {
                ...
                (0, utils_1.debugLog)('메모리 관리 설정 로드 완료');
            }
            else {
                await this.saveSettings();
                (0, utils_1.debugLog)('기본 메모리 관리 설정 생성 완료');
            }
        }
        catch (error) {
            (0, utils_1.errorLog)('메모리 관리 설정 로드 중 오류:', error);
        }
    }
    /**
     * 설정 파일 저장
    ...
    async saveSettings() {
        try {
            ...
            (0, utils_1.debugLog)('메모리 관리 설정 저장 완료');
        }
        catch (error) {
            (0, utils_1.errorLog)('메모리 관리 설정 저장 중 오류:', error);
        }
    }
    /**
     * 네이티브 메모리 모듈 초기화
    ...
    async initializeNativeMemory() {
        try {
            ...
            if (isAvailable) {
                ...
                if (started) {
                    (0, utils_1.debugLog)('네이티브 메모리 모니터링 시작됨');
                    // 초기 메모리 정보 수집
            ...
            (0, utils_1.errorLog)('네이티브 메모리 모듈 초기화 중 오류:', error);
        }
    }
    /**
     * 메모리 풀 초기화
    ...
    initializeMemoryPools() {
        try {
            ...
            (0, utils_1.debugLog)('메모리 풀 초기화 완료');
        }
        catch (error) {
            (0, utils_1.errorLog)('메모리 풀 초기화 중 오류:', error);
        }
    }
    /**
     * 주기적 메모리 모니터링 시작
    ...
    async performMemoryCheck() {
        try {
            ...
            (0, utils_1.errorLog)('메모리 체크 중 오류:', error);
        }
    }
    /**
     * 메모리 풀 업데이트
    ...
    updateMemoryPools(memoryInfo) {
        try {
            ...
            (0, utils_1.errorLog)('메모리 풀 업데이트 중 오류:', error);
        }
    }
    /**
     * 앱 이벤트 리스너 설정
    ...
    async getMemoryInfo() {
        try {
            ...
            (0, utils_1.errorLog)('메모리 정보 가져오기 중 오류:', error);
            return this.getMemoryInfoJS();
        }
    }
    /**
    ...
    async optimizeMemory() {
        ...
        try {
            ...
            if (isAvailable) {
                try {
                    ...
                    if (resetResult && beforeUsage && afterUsage) {
                        ...
                        method = 'native';
                        (0, utils_1.debugLog)('네이티브 메모리 최적화 완료, 해제된 메모리:', freedBytes);
                    }
                }
                catch (nativeError) {
                    (0, utils_1.debugLog)('네이티브 메모리 최적화 실패, JavaScript 폴백 사용:', nativeError);
                }
            }
            // JavaScript 폴백 최적화
            if (freedBytes === 0) {
            ...
            (0, utils_1.debugLog)(`메모리 최적화 완료: ${freedBytes} bytes 해제, ${durationMs}ms 소요`);
            return {
                freedBytes,
                durationMs,
                method,
            ...
            (0, utils_1.errorLog)('메모리 최적화 중 오류:', error);
            return {
                freedBytes: 0,
    ...
    async optimizeMemoryJS() {
        try {
            ...
            (0, utils_1.errorLog)('JavaScript 메모리 최적화 중 오류:', error);
            return 0;
        }
    }
    /**
    ...
    async updateSettings(newSettings) {
        try {
            ...
            (0, utils_1.debugLog)('메모리 설정 업데이트 완료');
        }
        catch (error) {
            (0, utils_1.errorLog)('메모리 설정 업데이트 중 오류:', error);
        }
    }
    /**
     * 메모리 풀 정보 가져오기
    ...
    cleanup() {
        ...
        (0, utils_1.debugLog)('메모리 관리자 정리 완료');
    }
}
// 전역 메모리 관리자 인스턴스
...
function registerMemoryIpcHandlers() {
    ...
    electron_1.ipcMain.handle('memory:getInfo', async () => {
        try {
            ...
            (0, utils_1.errorLog)('메모리 정보 조회 오류:', error);
            return {
                success: false,
    ...
    // 메모리 최적화 실행
    electron_1.ipcMain.handle('memory:optimize', async (_, level) => {
        try {
            const manager = getMemoryManager();
            ...
            (0, utils_1.errorLog)('메모리 최적화 오류:', error);
            return {
                success: false,
                error: error instanceof Error ? error.message : String(error)
            };
    ...
    electron_1.ipcMain.handle('memory:forceGc', async () => {
        try {
            ...
            if (global.gc) {
                global.gc();
                (0, utils_1.debugLog)('강제 가비지 컬렉션 실행됨');
            ...
            (0, utils_1.errorLog)('강제 가비지 컬렉션 오류:', error);
            return {
                success: false,
                error: error instanceof Error ? error.message : String(error)
            };
    ...
    electron_1.ipcMain.handle('memory:setThreshold', async (_, threshold) => {
        try {
            ...
            (0, utils_1.errorLog)('메모리 임계값 설정 오류:', error);
            return {
                success: false,
                error: error instanceof Error ? error.message : String(error)
            };
    ...
    (0, utils_1.debugLog)('메모리 관련 IPC 핸들러 등록 완료');
}
/**
 * IPC 핸들러 정리
 */
function cleanupMemoryIpcHandlers() {
    ...
    electron_1.ipcMain.removeHandler('memory:force-gc');
    electron_1.ipcMain.removeHandler('memory:set-threshold');
    (0, utils_1.debugLog)('메모리 관련 IPC 핸들러 정리 완료');
}
