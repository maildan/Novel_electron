...
function registerSettingsHandlers() {
    try {
        // Setup IPC 핸들러 등록 (setProcessingMode 등)
        settingsIpcHandlers_1.default.register();
        // Setup 관리자 초기화 및 IPC 핸들러 등록
...
async function setupAllHandlers() {
    ...
    try {
        ...
        debugLog(`모든 IPC 핸들러 등록 Completed. 등록된 핸들러: ${Array.from(handlersState.registeredHandlers).join(', ')}`);
        return true;
    }
    catch (error) {
