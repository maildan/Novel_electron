...
function createAppMenu(options) {
    ...
    submenu.push({ role: 'services' }, { type: 'separator' }, { role: 'hide' }, { role: 'hideOthers' }, { role: 'unhide' }, { type: 'separator' });
    if (options.showQuit) {
        submenu.push({
    ...
    return {
        label: options.appName || electron_1.app.getName(),
        submenu
    };
}
/**
 ...
 * 보기 메뉴 생성
 */
function createViewMenu(options) {
    const submenu = [
    ...
    if (options.showDevTools) {
        submenu.push({ type: 'separator' }, { role: 'toggleDevTools', label: '개발자 도구' });
    }
    return {
        label: '보기',
        submenu
...
function createWindowMenu(options) {
    ...
    if (process.platform === 'darwin') {
        submenu.push({ type: 'separator' }, { role: 'front', label: '모두 앞으로 가져오기' }, { type: 'separator' }, { role: 'window', label: '윈도우' });
    }
    return {
        label: '윈도우',
        submenu
 ...
 * 도움말 메뉴 생성
 */
function createHelpMenu(options) {
    const submenu = [
        {
            label: '온라인 도움말',
            click: () => electron_1.shell.openExternal('https://help.example.com')
        },
        {
            label: '키보드 단축키',
        ...
        {
            label: '문제 신고...',
            click: () => electron_1.shell.openExternal('https://github.com/example/issues')
        }
    ];
 ...
 */
function convertToMenuItemOptions(item) {
    const options = {
        label: item.label,
...
async function handleFileOpen() {
    try {
        ...
        if (!focusedWindow)
            return;
        const { canceled, filePaths } = await electron_1.dialog.showOpenDialog(focusedWindow, {
...
function showAboutDialog() {
    const focusedWindow = electron_1.BrowserWindow.getFocusedWindow();
    electron_1.dialog.showMessageBox(focusedWindow || electron_1.BrowserWindow.getAllWindows()[0], {
        type: 'info',
        title: `${menuOptions.appName} 정보`,
        message: menuOptions.appName || electron_1.app.getName(),
        detail: [
            `버전: ${electron_1.app.getVersion()}`,
 ...
 * 메뉴 업데이트
 */
function updateMenu(newOptions) {
    try {
        if (newOptions) {
...
function createContextMenu(options) {
    try {
        ...
        const contextMenu = electron_1.Menu.buildFromTemplate(template);
        if (options.window) {
            contextMenu.popup({
...
function setupMenuIpcHandlers() {
    // 메뉴 업데이트 요청
    electron_1.ipcMain.handle('menu:update', (event, options) => {
        updateMenu(options);
        return true;
    });
    // 컨텍스트 메뉴 생성 요청
    electron_1.ipcMain.handle('menu:show-context', (event, options) => {
    ...
    // 최근 파일 목록 조회
    electron_1.ipcMain.handle('menu:get-recent-files', () => {
        return recentFiles;
    });
    // 메뉴 액션 히스토리 조회
    electron_1.ipcMain.handle('menu:get-action-history', () => {
        return menuActionHistory.slice(0, 20);
    });
    // 최근 파일에 추가
    electron_1.ipcMain.handle('menu:add-recent-file', (event, filePath) => {
        addToRecentFiles(filePath);
        updateMenu();
        return true;
    });
