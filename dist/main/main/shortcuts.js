...
exports.toggleGlobalShortcut = toggleGlobalShortcut;
exports.registerLocalShortcut = registerLocalShortcut;
exports.initializeShortcuts = initializeShortcuts;
exports.setupDefaultShortcuts = setupDefaultShortcuts;
exports.getShortcutStats = getShortcutStats;
 ...
 * 단축키 충돌 검사
 */
function checkShortcutConflict(accelerator) {
    const conflicts = [];
    // 전역 단축키와 충돌 검사
 ...
 * 단축키 히스토리 추가
 */
function addToHistory(action, accelerator) {
 ...
 * 단축키 활성화/비활성화
 */
function toggleGlobalShortcut(accelerator, enabled) {
    try {
        const shortcutData = registeredShortcuts.get(accelerator);
        if (!shortcutData) {
            return false;
 ...
 * 입력과 단축키 매칭
 */
function matchShortcutInput(accelerator, input) {
    try {
        const parsed = parseShortcut(accelerator);
        if (!parsed.isValid)
            return false;
        ...
        // 수정자 키 매칭 확인
        const requiredCommandOrControl = parsed.modifiers.some(mod => mod === 'ctrl' || mod === 'control' || mod === 'commandorcontrol' || mod === 'cmd' || mod === 'command');
 ...
 * 기본 단축키 핸들러
 */
function handleDefaultShortcut(accelerator) {
