...
export namespace MemoryIpcTypes {
  /**
...
export namespace SystemIpcTypes {
  /**
   * 시스템 정보
   */
...
export namespace SettingsIpcTypes {
  /**
   * Settings 전용 IPC 응답 (기본 IpcResponse 확장)
   */
  export interface SettingsIpcResponse<T = any> extends IpcResponse<T> {
    message?: string;
   ...
   * 설정 다중 업데이트 요청
   */
  export interface SettingsMultipleUpdateRequest {
    updates: Array<{
      category: keyof AppSettings;
      key: string;
      value: any;
...
export namespace WindowIpcTypes {
  /**
   * 윈도우 생성 옵션
   */
...
export namespace NativeIpcTypes {
  /**
   * 네이티브 모듈 상태
   */
...
export namespace DialogIpcTypes {
  /**
   * 메시지 다이얼로그 옵션
   */
...
export interface IpcChannelMap {
  ...
  [CHANNELS.EXPORT_DATA]: {
    ...
    response: { filePath: string; recordCount: number };
  };
  [CHANNELS.IMPORT_DATA]: {
    request: DatabaseIpcTypes.DataImportParams;
    response: { success: boolean; importedRecords: number };
  };
  [CHANNELS.CLEAR_DATA]: {
  ...
  [CHANNELS.DB_HEALTH_CHECK]: {
    request: void;
    response: { status: 'healthy' | 'degraded' | 'critical'; issues?: string[] };
  };
  ...
  [CHANNELS.NATIVE_GET_GPU_MEMORY_STATS]: {
    request: void;
    response: { total: number; used: number; free: number };
  };
  [CHANNELS.NATIVE_RUN_GPU_ACCELERATION]: {
    request: { taskType: string; data: any };
  ...
  [CHANNELS.NATIVE_PROCESS_DATA_PARALLEL]: {
    request: { data: any[]; chunkSize?: number };
    response: { results: any[]; totalTime: number };
  };
  ...
  [CHANNELS.NATIVE_CALCULATE_FILE_HASH]: {
    request: { filePath: string; algorithm?: 'md5' | 'sha1' | 'sha256' };
    response: { hash: string };
  };
  [CHANNELS.NATIVE_CALCULATE_DIRECTORY_SIZE]: {
    request: { directoryPath: string };
    response: { size: number; fileCount: number };
  };
  [CHANNELS.NATIVE_CALCULATE_STRING_SIMILARITY]: {
    request: { str1: string; str2: string };
  ...
  [CHANNELS.NATIVE_START_PERFORMANCE_MEASUREMENT]: {
    ...
    response: { measurementId: string };
  };
  [CHANNELS.NATIVE_END_PERFORMANCE_MEASUREMENT]: {
    request: { measurementId: string };
  ...
  [CHANNELS.SETTINGS_GET]: {
    request: { category?: keyof SettingsIpcTypes.AppSettings; key?: string };
    response: any;
  };
  ...
  [CHANNELS.SETTINGS_UPDATE_MULTIPLE]: {
    ...
    response: { success: boolean; failedUpdates?: string[] };
  };
  [CHANNELS.SETTINGS_RESET]: {
    request: { category?: keyof SettingsIpcTypes.AppSettings };
    response: { success: boolean };
  };
  ...
  [CHANNELS.SETTINGS_LOAD]: {
    request: void;
    response: { success: boolean; settings: SettingsIpcTypes.AppSettings };
  };
  ...
  [CHANNELS.GET_METRICS_HISTORY]: {
    request: { startTime?: number; endTime?: number };
    response: any[];
  };
  ...
  [CHANNELS.GET_SYSTEM_HEALTH]: {
    request: void;
    response: { status: 'healthy' | 'warning' | 'critical'; metrics: any };
  };
  ...
  [CHANNELS.GET_APP_INFO]: {
    request: void;
    response: { name: string; version: string; description: string };
  };
 ...
 * IpcResponse 타입 가드
 */
export function isIpcResponse<T = any>(obj: any): obj is IpcResponse<T> {
  return (
    typeof obj === 'object' &&
    obj !== null &&
 ...
 * IpcError 타입 가드
 */
export function isIpcError(obj: any): obj is IpcError {
  return (
    typeof obj === 'object' &&
    obj !== null &&
...
export function isValidIpcChannel(channel: string): channel is IpcChannel {
  return Object.values(CHANNELS).includes(channel as any);
}
 ...
 * 실패 응답 생성 헬퍼
 */
export function createErrorResponse(error: string | IpcError): IpcResponse<never> {
 ...
 */
export type TypedIpcHandler<T extends keyof IpcChannelMap> = (
  event: Electron.IpcMainInvokeEvent,
  ...args: IpcRequestType<T> extends void ? [] : [IpcRequestType<T>]
) => Promise<IpcResponse<IpcResponseType<T>>>;

/**
 * 타입 안전한 IPC 호출 함수 타입
 */
export type TypedIpcInvoke = <T extends keyof IpcChannelMap>(
  channel: T,
  ...args: IpcRequestType<T> extends void ? [] : [IpcRequestType<T>]
) => Promise<IpcResponseType<T>>;
